...
- Iterating over particle neighbours: Finally, iterating over each particle's neighbours to perform SPH calculations typically has a complexity of $O(kn)$, where $k$ is the average number of neighbours per particle and $k < n$.
...
Up to this point, the algorithm's steps included logic that do not change during the time integration procedure, and therefore, the aformentioned functions are only used during initialisation (called at the end of `initialise()` in `SPH-main.cpp`). However, the next steps are steps that need to be followed in each iteration, as the particles' positions change.
